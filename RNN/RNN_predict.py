...
def main(_):
    ...
    print("start padding....")
    testX2=np.array([row + [0] * (FLAGS.sequence_length - len(row)) for row in testX]) #padding to max length
    print("end padding...")
   # 3.create session.
    ...
    with tf.Session(config=config) as sess:
        ...
        for start, end in zip(range(0, number_of_training_data, FLAGS.batch_size),range(FLAGS.batch_size, number_of_training_data+1, FLAGS.batch_size)):
            logits=sess.run(textRNN.logits,feed_dict={textRNN.input_x:testX2[start:end],textRNN.dropout_keep_prob:1}) 
